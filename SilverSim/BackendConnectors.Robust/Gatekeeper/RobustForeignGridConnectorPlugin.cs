...
namespace SilverSim.BackendConnectors.Robust.Gatekeeper
{
    ...
    public class RobustForeignGridConnectorPlugin : ServicePluginHelo, IForeignGridConnectorPlugin, IPlugin
    {
        ...
        public ForeignGridConnector Instantiate(string url)
        {
            return new RobustForeignGridConnector(url);
        }

        public bool IsAgentSupported(List<GridType> gridTypes)
        {
            foreach(GridType gridType in gridTypes)
            {
                if(gridType.Name == Name)
                {
                    return true;
                }
            }
